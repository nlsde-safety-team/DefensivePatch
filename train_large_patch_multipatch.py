...
# from dataloader import mydataset, mydataset_imagenet
import argparse
import vgg
import mobilenet
import shufflenet
...
def initialize_model(model_name):
    ...
    elif model_name == "vgg16_pat":
        """ VGG16
        """
        model = vgg.vgg16_bn(num_classes=10).cuda()
        model = nn.DataParallel(model)
        model.load_state_dict(torch.load('/root/yinzixin/defense_patch/defense_PAT/adv-plat/experiments/cifar_vgg16_bn_PAT_new/ckpt/model_final.pth'))
        model = model.module
    ...
    elif model_name == "mobilenet_pat":
        model = mobilenet.MobileNetV2(10).cuda()
        model = nn.DataParallel(model)
        model.load_state_dict(torch.load('/root/yinzixin/defense_patch/defense_PAT/adv-plat/experiments/cifar_mobilenet_PAT_new/ckpt/model_final.pth'))
        model = model.module

    elif model_name == "shufflenet":
        model = shufflenet.ShuffleNet(10).cuda()
        model.load_state_dict(torch.load('finetunes/shufflenet_best_new.pth'))
    elif model_name == "shufflenet_pat":
        model = shufflenet.ShuffleNet(10).cuda()
        model = nn.DataParallel(model)
        model.load_state_dict(torch.load('/root/yinzixin/defense_patch/defense_PAT/adv-plat/experiments/cifar_shufflenet_PAT_new/ckpt/model_final.pth'))
        model = model.module
    ...
    elif model_name == "resnet50_pat":
        """ Resnet50
        """
        model = resnet.resnet50(num_classes=10).cuda()
        model = nn.DataParallel(model)
        model.load_state_dict(torch.load('/root/yinzixin/defense_patch/defense_PAT/adv-plat/experiments/cifar_resnet50_PAT_new/ckpt/model_final.pth'))
        model = model.module

    elif model_name == "alexnet":
        """ Alexnet
        """
        model = alex.alexnet(num_classes=10).cuda()
        model.load_state_dict(torch.load('finetunes/alexnet_best_2.pth'))
...
# change the shape but do not increase or decrease the number of the elements
...
def PCA(fs, k=10):
    batch, channel, H, W = fs.shape
    X = fs.view(batch, channel, H * W)
    X_mean = X.mean(dim=(1,2), keepdim=True)
    X_mean = X_mean.repeat((1, channel, H*W))
    X = X - X_mean.expand_as(X)
    
    X_t = X.transpose(1, 2)
    U, S, V = torch.svd(X_t)
        
    X_PCA = torch.matmul(X, U[:, :, :k])
...
# save_image(defense_patch, 'raw_patch.png')
...
def train_patch(defense_patch, model):
    ...
    for epoch in range(1):
        ...
        for i, data in enumerate(tqdm_bar):
            ...
            loss_f = 0 # cirterion(outputs_patch, labels)
            loss_p = (-torch.log(prob_index + 1e-10) * torch.eye(batchsize).to(device)).sum() / batchsize
            loss = loss_f + loss_p
...
# model_1 = initialize_model('resnet50')
...
#         if count in [46]:
#             print(name)
#             m.register_forward_pre_hook(viz_collection)
#         count += 1


# model_2 = initialize_model('alexnet')
...
# model_2.eval()
# count = 0
# for name, m in model_2.named_modules():
# #     print(name)
#     if isinstance(m, torch.nn.Conv2d):
#         # print(count, name)
#         if count in [9]:
...
if args.prior != 'None':
    defense_patch = torch.load('new_logs/fusion-%s+epoch-5+layer-9+loss-CE_mask+/prior.pkl' % (args.prior))
...
for epoch in range(30):
    ...
    # defense_patch_1 = train_patch(defense_patch=defense_patch.clone(), model=model_1)
    # defense_patch_2 = train_patch(defense_patch=defense_patch.clone(), model=model_2)
    ...
    for i, data in enumerate(tqdm_bar):
        ...
        for idx, model in enumerate(models):
            ...
            prob_index = prob.index_select(1, labels)
            loss_p += 0 # (-torch.log(prob_index + 1e-10) * torch.eye(batchsize).to(device)).sum() / batchsize

            model_outputs_s.append(model_outputs)

        # defense_inputs_1 = get_inputs(inputs, labels, defense_patch_1)
        # defense_inputs_2 = get_inputs(inputs, labels, defense_patch_2)

        # features = []
        # model_1_output_defense_inputs = model_1(defense_inputs)
        # model_2_output_defense_inputs = model_2(defense_inputs)
        # model_1_output_defense_inputs_1 = model_1(defense_inputs_1)
        # model_2_output_defense_inputs_2 = model_2(defense_inputs_2)
        ...
        # features.append(defense_inputs)
        # features.append(defense_inputs)
        # features.append(defense_inputs_1)
        # features.append(defense_inputs_2)

        loss_f = loss_feature(features, labels)


        # prob_1 = F.softmax(model_1_output_defense_inputs, dim=1)
        # prob_index_1 = prob_1.index_select(1, labels)

        # prob_2 = F.softmax(model_2_output_defense_inputs, dim=1)
        # prob_index_2 = prob_2.index_select(1, labels)

        # loss_p = (-torch.log(prob_index_1 + 1e-10) * torch.eye(batchsize).to(device)).sum() / batchsize + \
        #          (-torch.log(prob_index_2 + 1e-10) * torch.eye(batchsize).to(device)).sum() / batchsize
        ...
        # predicted = torch.max(model_2_output_defense_inputs, 1)[1]
